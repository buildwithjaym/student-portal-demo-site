...
function TeacherGradesContent() {
  ...
  const gradingPeriodFromQuery =
    (searchParams.get('gradingPeriod') as GradingPeriod | null) || null
  ...
  const [selectedGradingPeriod, setSelectedGradingPeriod] = useState<GradingPeriod>(
    gradingPeriodFromQuery ?? '1st'
  )
  ...
  const loadWindowsAndCurrentPeriod = async (schoolYear: string) => {
    ...
    if (gradingPeriodFromQuery) return
